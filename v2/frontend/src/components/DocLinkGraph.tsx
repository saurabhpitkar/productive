...
interface LinkRef { id: string; label: 'up' | 'requires' }

interface Props {
  docId:        string
  docName:      string
  docStatus:    string
  parentLinks:  LinkRef[]   // shown above root; 'requires' → arrow, 'up' → line only
  childLinks:   LinkRef[]   // shown below root; 'requires' → arrow, 'up' → line only
...
interface Edge {
  ...
  arrow: boolean  // false = line only (for 'up' relationships)
}
...
export function DocLinkGraph({ docId, docName, docStatus, parentLinks, childLinks, onDocClick, compact = false, className }: Props) {
  ...
  // Docs reachable as grandchildren of 'up' children — suppress these from being shown as direct children
  ...
  const upChildGrandchildIds = new Set<string>(
    childLinks.filter(l => l.label === 'up').flatMap(l => l2Req[l.id] ?? [])
  )
  ...
  for (const { id, label } of childLinks) {
    if (seen.has(id)) continue
    // Skip 'requires' children that are already reachable as grandchildren of an 'up' child
