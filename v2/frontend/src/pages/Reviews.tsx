...
export function Reviews() {
  ...
  const handleResolve = async (id: string, outcome: 'approved' | 'rejected' | 'cancelled') => {
    setResolving(true)
    try {
        ...
        <div className="flex items-center gap-3">
          ...
          {!loading && (
            <span className="text-sm text-gray-500 dark:text-gray-400">
              {reviews.length} {tab === 'pending' ? 'pending' : 'total'}
            </span>
          )}
        </div>
        <button
                    ...
                    <>
                      ...
                      {r.outcome !== null && (
                        <div className="text-sm text-gray-500 dark:text-gray-400">
                          ...
                          {r.human_notes && (
                            <p className="mt-1 italic">"{r.human_notes}"</p>
                          )}
                        </div>
                      )}
                    </>
