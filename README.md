...
| Your data stored as plain markdown files | ❌ | ✅ |
...
![Productive UI — Active Docs, linked doc view, AI assistant, and flow map](assets/demo.png)

---

## Quick start

**Prerequisites:** Docker Desktop, a Google or GitHub OAuth app, and an Anthropic or Google AI API key.
...
APP_ORIGIN_V3=https://your-domain.com
...
Open `http://localhost:3003` (or your domain). Sign in with GitHub or Google. On first login you'll be offered the demo vault.
...
4. Restart: `docker compose up -d backend-v3`
...
| `linked_doc_ids` | UUID[] | IDs of docs this doc links to (derived from links) |
...
| GET | `/ai/usage` | 7-day token usage aggregated by model |
...
Restart: `docker compose up -d mcp-server-v3`
...
Add a public hostname for port 3004 in your Cloudflare Zero Trust dashboard (e.g. `mcp.your-domain.com → localhost:3004`), or add a second ingress rule to your existing tunnel config.
...
For **Claude Code** specifically, add the above to `.claude/settings.json` in your project (or `~/.claude/settings.json` globally).
...
```
Docker Compose (productive-net-v3)
├── frontend-v3   React 18 + Vite PWA (port 3003)
│                 Offline-first via IndexedDB (Dexie.js)
│                 Delta sync every 3 minutes (configurable)
├── backend-v3    Rust (Axum) — no port exposed externally
│                 Docs stored as plain markdown files with YAML frontmatter
│                 Per-user git history for every write
├── mcp-server-v3 Python MCP server (port 3004, SSE transport)
│                 Calls backend-v3 via PAT — no direct file access
└── cloudflared   Cloudflare Tunnel (public access without open ports)
```

**Auth:** Google OAuth 2.0 or GitHub OAuth (browser), or PAT Bearer token (agents/API).  
**AI:** Claude (Anthropic) or Gemini (Google) — user-configured, key encrypted at rest with Fernet.  
**Data:** Each user has their own isolated directory of markdown files. No shared tables.
