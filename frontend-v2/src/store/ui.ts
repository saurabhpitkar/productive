...
const LAST_DOC_KEY       = 'pa:last-doc'
const SIDEBAR_PINNED_KEY = 'pa:sidebarPinned'

function getSavedLastDoc(): string | null {
...
export const useUIStore = create<UIStore>(set => ({
  ...
  openPanel: docId => set(() => {
    const id = docId ?? 'new'
    if (id !== 'new') {
      try { localStorage.setItem(LAST_DOC_KEY, id) } catch {}
    }
    return { panelDocId: id }
  }),
