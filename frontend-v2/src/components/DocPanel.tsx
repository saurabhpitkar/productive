...
const LABEL_OPTS: LinkLabel[] = ['up', 'requires', 'related_to']
const LABEL_COLORS: Record<LinkLabel, string> = {
  up:         'bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300',
...
export function DocPanel({ docId, onClose }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isNew) {
        const newDoc = await createDoc({ name: name.trim(), body, flag, due_date, due_time, priority: pri, status, list_id, tags })
