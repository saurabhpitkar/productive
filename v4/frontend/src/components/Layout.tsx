...
const SIDEBAR_PCT     = 15   // vw when expanded
const AI_MIN_PCT      = 15   // vw default / minimum for AI panel
const AI_MAX_PCT      = 40   // vw maximum for AI panel (user can drag up to this)
const PANEL_MIN_PCT   = 25   // vw hard minimum for doc/empty panel during drag
const DOCLIST_MIN_PCT = 20   // vw minimum for the doc list column

function getPanelDefault(sidebarOpen: boolean, aiOpen: boolean): number {
  if (aiOpen) return sidebarOpen ? 50 : 60
  return sidebarOpen ? 60 : 75
}
...
export function Layout() {
  const { sidebarOpen, sidebarPinned, toggleSidebar, panelDocId, closePanel, syncInterval, activeRoutingCount } = useUIStore()
  const isMobile = useIsMobile()

  const [panelVw,         setPanelVw]         = useState(() => getPanelDefault(sidebarOpen, false))
  const [aiVw,            setAiVw]            = useState(AI_MIN_PCT)
  const [aiOpen,          setAiOpen]          = useState(false)
  const [searchOpen,      setSearchOpen]      = useState(false)
  const [inboxOpen,       setInboxOpen]       = useState(false)
  const [inboxDefaultMode, setInboxDefaultMode] = useState<'ai' | 'manual'>('ai')
  const [wakeLockOn,      setWakeLockOn]      = useState(false)
  const wakeLockRef = useRef<WakeLockSentinel | null>(null)
  const mainColRef  = useRef<HTMLDivElement>(null)
  const swipeStart  = useRef<{ x: number; y: number }>({ x: 0, y: 0 })
  // Ref so the snap effect always reads the latest aiVw without causing re-runs on drag
  const aiVwRef = useRef(AI_MIN_PCT)
  aiVwRef.current = aiVw

  const showPanel = panelDocId !== null
  const wakeLockAvailable = 'wakeLock' in navigator

  // Snap panelVw to the user-specified defaults whenever sidebar or AI open/close.
  // Does NOT include aiVw in deps — drag-resizing the AI panel must not re-snap.
  useEffect(() => {
    const sidebarW = sidebarOpen ? SIDEBAR_PCT : 0
    const curAiW   = aiOpen ? aiVwRef.current : 0
    const available = 100 - sidebarW - curAiW
    const def = getPanelDefault(sidebarOpen, aiOpen)
    setPanelVw(Math.max(PANEL_MIN_PCT, Math.min(available - DOCLIST_MIN_PCT, def)))
  }, [sidebarOpen, aiOpen]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // ── Panel drag-resize (left edge of right panel) ───────────────────────────
  const startResize = (e: React.MouseEvent) => {
    ...
    const curAiW    = aiOpen ? aiVwRef.current : 0
    const available = 100 - sidebarW - curAiW
    const maxPct    = available - DOCLIST_MIN_PCT
  ...
  // ── AI panel drag-resize (left edge of AI panel) ───────────────────────────
  const startAiResize = (e: React.MouseEvent) => {
    e.preventDefault()
    const startX      = e.clientX
    const startAiPct  = aiVwRef.current
    const startPanelPct = panelVw
    const sidebarW    = sidebarOpen ? SIDEBAR_PCT : 0

    const onMove = (ev: MouseEvent) => {
      const deltaPct = (startX - ev.clientX) / window.innerWidth * 100
      const newAi    = Math.max(AI_MIN_PCT, Math.min(AI_MAX_PCT, startAiPct + deltaPct))
      const available = 100 - sidebarW - newAi
      const newPanel  = Math.max(PANEL_MIN_PCT, Math.min(available - DOCLIST_MIN_PCT, startPanelPct - deltaPct))
      setAiVw(newAi)
      setPanelVw(newPanel)
    }
    const onUp = () => {
  ...
  // ── Capture helpers ────────────────────────────────────────────────────────
  const openCapture = (mode: 'ai' | 'manual') => {
    setInboxDefaultMode(mode)
    setInboxOpen(true)
  }

  // ── AI toggle: reset AI panel width, auto-collapse sidebar unless pinned ───
  const handleAiToggle = () => {
    const next = !aiOpen
    if (next) {
      setAiVw(AI_MIN_PCT)
      if (sidebarOpen && !sidebarPinned) toggleSidebar()
    }
    setAiOpen(next)
  }
  ...
  return (
    <div className="flex h-[100dvh] overflow-hidden bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 pt-safe pl-safe pr-safe pb-safe">
      ...
      {/* Inbox capture overlay — mobile only (desktop renders inline in panel) */}
      {isMobile && inboxOpen && (
        <InboxCapture
          onClose={() => setInboxOpen(false)}
          defaultMode={inboxDefaultMode}
        />
      )}
      ...
      <div ref={mainColRef} className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="flex items-center gap-2 px-4 h-14 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 flex-shrink-0">
          <div className="relative flex-shrink-0">
            ...
            {activeRoutingCount > 0 && (
              <span className="absolute -top-1 -right-1 min-w-[1rem] h-4 px-1 rounded-full bg-indigo-500 text-white text-[0.6rem] font-bold flex items-center justify-center leading-none">
                {activeRoutingCount}
              </span>
            )}
          </div>
          ...
          <span className="font-semibold text-indigo-600 dark:text-indigo-400 text-sm tracking-wide flex-shrink-0">
            Productive
            <span className="ml-1.5 text-[0.6rem] font-bold bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 px-1.5 py-0.5 rounded-full align-middle">v4</span>
          </span>
          ...
          {/* New doc - opens Capture in Manual mode */}
          <button
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden relative">
          ...
          {/* Doc list - flex-1 fills remaining width */}
          <main
          ...
          {/* Desktop: panel drag handle + right panel + AI drag handle + AI panel */}
          {!isMobile && (
            <>
              {/* Panel drag handle */}
              <div
              ...
              {/* Right panel: capture (inline), doc view, or empty state */}
              <div
